...
<body>
			...
			<form method="GET">
				<td>
					<select name="pd">
						...
						?>
						<option value="<?php echo $mostrar['ID_platillo']; ?>"><?php echo $mostrar['ID_platillo']; ?></option>
					<?php } ?>
					</select>
				</td>
				<td>
	...
	<table>
		<tr>
			<th>Platillo</th>
			<th>Existencia</th>
			<th colspan="2">Nuevo ingreso</th>
		</tr>
		<?php
		$idtemp = $_GET['pd'];
			$sql = "SELECT * FROM platillo WHERE ID_platillo LIKE '$idtemp%'";
			$resultado = mysqli_query($link,$sql);
			while ($mostrar=mysqli_fetch_array($resultado)) {
		?>
		<tr>
			...
			<form method="POST" action="actualizar_existencia.php">
				<aside style="display: none;">
					<input type="text" name="id" value="<?php echo $mostrar['ID_platillo']; ?>">
				</aside>
				<td><input type="number" name="extra" required min="1"></td>
