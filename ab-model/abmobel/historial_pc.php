	...
	<ul id="listPedidos">
        <?php
            ...
            while ($arrayLapso = mysqli_fetch_array($tablaLapso)) {
                ...
                $queryPedidos = "SELECT Platillo, Cantidad, Hora, Status FROM pedido WHERE Cliente = '" . $correoCliente . "' AND (Status = -1 OR Status = 5) AND Fecha = '" . $fecha . "' AND Hora BETWEEN '" . $horaEntrada . "' AND '" . $horaSalida . "'";//Historial
...
<style type="text/css">
	*{
    ...
    div.estatus{
    	background-color: red;/*Historial*/
    	text-align: center;
    	width: 20%;
    	float: right;
    }
